...
void addProduct(struct Shop *s, int *size, float *revenuePtr){
	if(*revenuePtr <= 0){
		printf("\nBan khong co tien de nhap hang\n\n");
	}
	else{
		(*size)++;
		s = realloc(s, (*size)*sizeof(struct Shop));
		printf("\nNhap ID san pham: ");
		scanf("%d", &s[(*size) - 1].id);
		fflush(stdin);
		...
		printf("\n");
		(*revenuePtr) -= s[(*size) - 1].quantity * s[(*size) - 1].importPrice;
	}
}
...
void editProduct(struct Shop *s, int *size, float *revenuePtr){
	...
	for(int i = 0; i < *size; i++){
		if(id == s[i].id){
			...
			if(quantity > s[i].quantity){
				(*revenuePtr) += (quantity - s[i].quantity) * s[i].importPrice;
			}
				...
				if((*revenuePtr) <= 0){
					...
					(*revenuePtr) -= (s[i].quantity - quantity) * s[i].importPrice;
				}
			}
...
void newProducts(struct Shop *s, int *size, float *revenuePtr){
	int id, found = 0, quantity;
	printf("\nNhap ID san pham: ");
	scanf("%d", &id);
	for(int i = 0; i < (*size); i++){
		if(id == s[i].id){
			...
			scanf("%d", &s[i].quantity);
			if(quantity > s[i].quantity){
				printf("\nHay nhap so luong hang them hang\n\n");
			}
			else{
				printf("\nNhap them so luong hang hoan tat\n\n");
				(*revenuePtr) -= (s[i].quantity - quantity) * s[i].importPrice;
	...
	if(found == 0){
		if((*revenuePtr) <= 0){
			printf("\nBan khong du tien de nhap hang\n\n");
		}
		else{
			fflush(stdin);
			(*size)++;
	...
	if((*size) == 0){
		...
		switch(choice){
			case 2:
				...
				break;
			case 3:
				newProducts(s, size, revenuePtr);
				break;
			case 4:
				editProduct(s, size, revenuePtr);
				break;
			case 5:
				sortByPrice(s, size);
				break;
